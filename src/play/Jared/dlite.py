...
class Dlite():
    ...
    def GetNeighborList(self,X):
        ...
        #if x>=0 and x<self.x and y+factor>=0 and y+factor<self.y:
        #    states.append(self.grid[x][y+factor])
            
        #if x>=0 and x<self.x and y-factor>=0 and y-factor<self.y:
        #    states.append(self.grid[x][y-factor])


        #if x+factor>=0 and x+factor<self.x and y+factor>=0 and y+factor <self.y:
        #    states.append(self.grid[x+factor][y+factor])
        #if x-factor>=0 and x-factor<self.x and y-factor>=0 and y-factor <self.y:
        #    states.append(self.grid[X.pt.x-factor][X.pt.y-factor])
        #if x-factor>=0 and x-factor<self.x and y+factor>=0 and y+factor <self.y:
        #    states.append(self.grid[X.pt.x-factor][X.pt.y+factor])
        #if x+factor>=0 and x+factor<self.x and y-factor>=0 and y-factor <self.y:
        #    states.append(self.grid[X.pt.x+factor][X.pt.y-factor])
    ...
    def Succor(self,u):
        ...
        for s in self.GetRealNeighbors(u):
            if s.g < u.g:
                #print "Added", u.pt.x,u.pt.y, s.pt.x,s.pt.y,s.g,u.g
                out.append(s)
            #else:
                #print "DOT G", u.pt.x,u.pt.y, s.pt.x,s.pt.y,s.g,u.g
        if not out:
            #print "neighbors lel", len(self.GetRealNeighbors(u))
