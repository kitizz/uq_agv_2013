...
def main(map):
    ...
    board = {}
    print map.getHeight() * map.getWidth()
    for i in range(1, map.getHeight(), 1): 
        for k in range(1, map.getWidth(), 1):
            ans = (map.getImage()[i][k] == ([255,255,255]))
            if all(ans):
                board[i] = None
    ...
    ans, goal = aStarSearch(board,vehiclemoves)
